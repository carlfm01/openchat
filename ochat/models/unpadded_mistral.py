...
class UnpaddedMistralYaRNScaledRotaryEmbedding(nn.Module):
    """MistralRotaryEmbedding extended with YaRN for unpadded models. See: https://arxiv.org/abs/2309.00071"""
    def __init__(self, dim, max_position_embeddings=16384, base=10000, scale=1, original_max_position_embeddings=8192,
    ...
    def forward(self, seq_len=None, device=None, dtype=None):
        # Adjust the `if` block for `forward` in `UnpaddedMistralModel`.
...
class UnpaddedMistralModel(UnpaddedMistralPreTrainedModel):
    ...
    def __init__(self, config: MistralConfig):
        ...
        if rope_scaling_type == "yarn":
            print(f"Initializing YaRN.")
            self.rotary_emb = UnpaddedMistralYaRNScaledRotaryEmbedding(
                dim=config.hidden_size // config.num_attention_heads,
                max_position_embeddings=config.max_position_embeddings,
                base=config.rope_theta,
                scale=config.rope_scaling["factor"],
                original_max_position_embeddings=config.rope_scaling["original_max_position_embeddings"],
                extrapolation_factor=1,  # Adjust if needed
                attn_factor=1,  # Adjust if needed
                beta_fast=32,  # Adjust if needed
                beta_slow=2,  # Adjust if needed
                finetuned=config.rope_scaling["finetuned"]
            )
        else:
